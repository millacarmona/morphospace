...
Another element that can be projected and visualized in morphospaces are morphometric axes, i.e. a synthetic axis built as a linear combination of shape variables. The following chunk illustrates how to combine the pipe workflow with the `proj_axis` function called outside the pipe to compute and project the first PC of each species (i.e. the morphometric axis of maximum intraspecific variation within each species).
