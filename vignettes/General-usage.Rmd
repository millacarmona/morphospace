...
msp3 <- mspace(shapes = detr_shapes, links = wf, cex.ldm = 5) %>% 
  proj_shapes(shapes = detr_shapes, col = spp) %>%
  proj_shapes(shapes = sp_shapes, bg = 1:nlevels(spp), pch = 21) %>%
  proj_groups(shapes = detr_shapes, groups = spp) %>% 
...
#corresponding to groups mean shapes and classification, the phylogenetic structure and scores for nodes and tips.
...
mspace(detr_shapes, links = wf, cex.ldm = 5) %>%
  proj_shapes(shapes = detr_shapes, col = spp) %>%
  proj_groups(shapes = detr_shapes, groups = spp) %>%
  proj_shapes(shapes = sp_shapes, pch = 21, bg = 1:13, cex = 1.2) %>%
