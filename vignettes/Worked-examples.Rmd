...
mspace(shapes, template = curvs, col.ldm = "black", cex.ldm = 5, plot = FALSE) %>%
  ...
  proj_consensus(shapes = expected_shapes(shapes, spxcac), 
                 bg = rep(c("blue", "orange"), each = 2), 
                 pch = rep(c(16, 1), times = 2), cex = 3) %>%
...
for(i in 1:2) {
  ...
  subdetr_shapes1 <- lm(two.d.array(subshapes) ~ subsex * subline * subreplica) %>%
    detrend_shapes %>% arrayspecs(p = 9, k = 2)

  subdetr_shapes2 <- lm(two.d.array(subdetr_shapes1) ~ subsizes) %>%
    detrend_shapes(xvalue = max(subsizes)) %>% arrayspecs(p = 9, k = 2)

  subdetr_shapes3 <- lm(two.d.array(subdetr_shapes2) ~ subcactus) %>%
    detrend_shapes(xvalue = c("Op", "Tr")[i]) %>% arrayspecs(p = 9, k = 2)
...
mspace(detr_shapes, template = curvs, col.ldm = "black",
  ...
  proj_groups(groups = spxcac, ellipse = TRUE, 
              col = rep(c("blue", "orange"), each = 2), 
              lty = rep(c(1, 2), times = 2), lwd = 2) %>%
  proj_consensus(shapes = expected_shapes(detr_shapes, spxcac), 
                 bg = rep(c("blue", "orange"), each = 2), 
                 pch = rep(c(1, 16), times = 2), cex = 3) %>%
  plot_mspace(legend = TRUE, cex.legend = 1.5)
```
...
mspace(shapes, bg.model = "light gray", plot = FALSE) %>%
  ...
  proj_consensus(shapes = expected_shapes(shapes, species), col = 1:4, pch = 21) %>% 
...
for(i in 1:nlevels(species)) {
  index <- species == levels(species)[i]
  subdetr_shapes <- lm(shapes[index,] ~ sizes[index]) %>% detrend_shapes(xvalue = max(sizes[index]))
...
# Generate stratomorphospace
strmsp <- mspace(detr_shapes, FUN = bg_prcomp, groups = species,
  ...
  proj_consensus(shapes = shapes_agesxspecies, pch = 16) %>%
  proj_groups(shapes = detr_shapes, groups = sp.bz, col = adjustcolor(1, alpha.f = 0))


#plot anagenetic trajectory in morphospace
for(i in 1:4) {
  index <- grepl(x = rownames(strmsp$gr_centroids), pattern = levels(species)[i])
  points(strmsp$gr_centroids[index,], type = "l", lwd = 2, col = i)
}
```

```{r}
# Plot time axis vs first PC
plot_mspace(mspace = strmsp, x = ages, axes = 1, col.points = species, xlab = "Time (MA)",
            groups = FALSE, mshapes = TRUE)


# Compute and arrange absolute ages for each group, then
# use it to add and connect mean shapes
xy <- cbind(tapply(ages, sp.bz, mean), strmsp$gr_centroids[,1])
xy <- xy[order(xy[,1]),]
for(i in 1:4) {
...
# compue consensus of each species
...
mspace(shapes, cex.ldm = 2, adj_frame = c(0.9, 0.85), plot = FALSE) %>%
  proj_shapes(shapes = shapes, bg = species, pch = 21, 
              cex = (sizes/max(sizes))^3) %>%
  proj_groups(shapes = shapes, groups = species, ellipse = TRUE, alpha = 0.2) %>%
  proj_consensus(sp_shapes, pch = 21) %>%
...
for(i in 1:nlevels(species)) {
  ...
  if(!any(i == c(4, 5))) {
    detr_shapes[,,index] <- lm(subshapes_mat ~ subind * subzone * subloc) %>%
      detrend_shapes %>% arrayspecs(p = 90, k = 3)
  } else {
    if(i == 4) {
...
for(i in 1:nlevels(species)) {
  proj_axis(obj = pls_list[[i]], mspace = allomsp, lwd = 3, col = i, type = "b",
...
```{r eval = F}
# Create allomorphospace using PLS to emphasize allometric variation 
# (this could take a while)
allomsp2 <- mspace(detr_shapes, FUN = pls_shapes, X = sizes, LOOCV = TRUE, 
               axes = 1, bg.model = "gray", cex.ldm = 0, template = detr_meanmesh, 
               invax = 1, plot = FALSE)


# Use plot_mspace to create hybrid morphospace with shape against logsize, 
# add legend
plot_mspace(allomsp2, x = sizes, axes = 1, col.points = species, 
            xlab = "log-size", xlim = range(sizes), adj_frame = c(0.9, 0.85), 
            points = FALSE, groups = FALSE)
title(' "Allomorphospace"')


# Add fitted values for each specimen
for(i in 1:nlevels(species)) {
  index <- species == levels(species)[i]
  mod <- lm(allomsp2$x[index,1] ~ sizes[index])
