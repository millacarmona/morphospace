...
msp <- mspace(detr_shapes, FUN = bg_prcomp, groups = species,
  ...
  proj_shapes(shapes = shapes_agesxspecies, pch = 16) %>%
...
strmsp <- mspace(detr_shapes, FUN = bg_prcomp, groups = species,
                 mag = 0.5, bg.model = "light gray", plot = FALSE) %>%
    proj_shapes(shapes = detr_shapes, pch = 1, col = species)
```

```{r, echo = FALSE, fig.show='hide', warning=FALSE}
# Generate stratomorphospace
strmsp <- mspace(detr_shapes, FUN = bg_prcomp, groups = species,
                 mag = 0.5, bg.model = "light gray", plot = TRUE) %>%
    proj_shapes(shapes = detr_shapes, pch = 1, col = species)
```

```{r}
# Plot time axis vs first PC
plot_mspace(mspace = strmsp, x = ages, axes = 1)

# Compute and arrange mean PC1 scores and absolute ages for each group, then
# use it to add and connect mean shapes
xy <- cbind(tapply(ages, sp.bz, mean), 
            proj_eigen(shapes_agesxspecies, 
                       vectors = strmsp$ordination$rotation, 
                       center = strmsp$ordination$center)[,1])
xy <- xy[order(xy[,1]),]
for(i in 1:4) {
...
allomsp2 <- mspace(detr_shapes, FUN = pls_shapes, X = sizes, LOOCV = TRUE, 
                   ...
                   invax = 1, plot = FALSE) %>% proj_shapes(detr_shapes)
...
for(i in 1:nlevels(species)) {
  index <- species == levels(species)[i]
  mod <- lm(allomsp2$projected$scores[index,1] ~ sizes[index])
