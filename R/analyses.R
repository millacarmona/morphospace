...
bg_prcomp <- function(x, groups, gweights = TRUE,
  ...
  if(LOOCV == FALSE) {
    ...
    return(list(sdev = sqrt(svd$d), rotation = svd$v, x = scores,
                center = grandmean, grcenters = x_gmeans))
  } else {
...
pls2b <- function(y, x, LOOCV = FALSE, recompute = FALSE) {
  ...
  if(LOOCV == FALSE) {
    ...
    return(list(yscores = yscores, yrotation = svd$v, ycenter = y_center,
                xscores = xscores, xrotation = svd$u, xcenter = x_center,
                values = svd$d))
  } else {
